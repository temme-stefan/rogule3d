...
import {GameVisualisation3D} from "./pages/GameVisualisation3D.tsx";
...
function App() {
    ...
    return (<>
        <main>
            ...
            {state?.state === "generating" && <div>Generating Game...</div>}
            {state?.state === "playing" && dimensions === "2d" &&
                <GameVisualisation2D game={game!} state={state!} handleInput={handleInput}/>}
            {state?.state === "playing" && dimensions === "3d" &&
                <GameVisualisation3D game={game!} state={state!} handleInput={handleInput}/>}
            {(state?.state === "win" || state?.state === "lose") &&
